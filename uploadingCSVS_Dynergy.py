...
def CreateCSVs(date2TryE,date2TryW,date2TryG,file2Try):
	myQueryHeaders="SELECT 'Property','SN','Channel','TimeStamping','Utility','Description','Value','Unit' FROM Reg5MinReadings UNION "
	myQueryE="SELECT (SELECT name FROM projectInfo limit 1) AS 'Property',SN,'1',TimeStamping,'Electrical',MeterParam,value,'kwh' FROM Reg5MinReadings WHERE DATE(TimeStamping)='%s' UNION "%(date2TryE)
	myQueryW="SELECT (SELECT name FROM projectInfo limit 1) AS 'Property',SN,'1',TimeStamping,'Water',MeterParam,value,'m3' FROM Reg5MinReadingsW WHERE DATE(TimeStamping)='%s' UNION "%(date2TryW)
	myQueryG="SELECT (SELECT name FROM projectInfo limit 1) AS 'Property',SN,'1',TimeStamping,'Gas',MeterParam,value,'m3' FROM Reg5MinReadingsG WHERE DATE(TimeStamping)='%s'"%(date2TryG)
	myQueryCSV="INTO OUTFILE '%s'  FIELDS TERMINATED BY ',' OPTIONALLY ENCLOSED BY '\"'   LINES TERMINATED BY '\n'"%(file2Try)
